 ...
 *
 * O campeão vem primeiro e grande: aqui ele é quem mais levou dedo na cara, o
 * que é uma vitória estranha e por isso mesmo a melhor piada da mesa. Empate no
  ...
  return (
    <Shell
      ...
      faixa={
        ...
        >
          {campeoes.length === 0
            ? 'A partida acabou sem ninguém levar carta nenhuma.'
            : souCampeao
              ? 'Foi você quem mais levou dedo na cara. Parabéns, eu acho.'
              : `${listar(campeoes.map((campeao) => campeao.apelido))} ${
                  campeoes.length > 1 ? 'levaram' : 'levou'
                } mais dedo na cara que todo mundo.`}
        </FaixaDeFase>
      }
      aoSair={aoSair}
    >
        ...
        <div className="flex flex-col gap-5">
                ...
                {campeoes.map((campeao) => (
                  <li key={campeao.id} className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-apoio text-texto-2 tabular-nums">
                      {campeao.pontos} {campeao.pontos === 1 ? 'carta' : 'cartas'}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <section className="rounded-papel border border-linha bg-superficie p-4">
            <div className="flex items-baseline justify-between gap-2">
              ...
              <p className="text-rotulo text-texto-3 uppercase">
                {dedo.rodada} {dedo.rodada === 1 ? 'carta' : 'cartas'}
              </p>
            </div>
