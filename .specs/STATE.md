...
- **Feature em andamento AGORA**: `hub-selecao-jogos` — `.specs/features/hub-selecao-jogos/` (`spec.md` confirmado pelo dono; `design.md` **escrito e completo** — Architecture Overview, Code Reuse, Components/Interfaces, Data Models, Error Handling, Risks, Tech Decisions; arquitetura registrada como `AD-013` acima). Ainda na branch **`feat/pacotes-avancados`** (não trocou de branch entre as features — considerar se cria branch própria antes de começar Tasks/Execute). Segunda de três rodadas combinadas do roadmap: `pacotes-avancados` (✅ fechada, ver abaixo) → **hub de seleção de jogos (em andamento — Design concluído, Tasks é o próximo passo)** → jogo Espião → (futuro) Cartas Contra a Turma. Roadmap completo e decisões de processo (usar `/impeccable shape` em vez de Claude Design nesta fase; usuário também usa Antigravity no mesmo repo) estão na memória pessoal do Claude Code deste usuário, arquivos `resenha-roadmap-jogos`, `resenha-design-iteracao-ao-vivo`, `resenha-ferramentas-externas-quebram-disciplina`, `resenha-gamificacao-feedback-expressivo` — **ler essa memória ao retomar**, ela não está no repo.
- **`tasks.md` aprovado pelo dono** (9 tasks, 3 fases) e execução em 2 lotes de sub-agent também aprovada. **Lote A (T1–T6, servidor) concluído e integrado** de volta em `feat/pacotes-avancados` via merge (`dd0aee6`, com um conflito trivial de checkboxes em `tasks.md` resolvido a favor da versão do worker). Commits do Lote A: `12155cf` (T1, protocolo), `935cbe7` (T2, catálogo), `e1ea325` (T3, registro), `dcc7a20` (T4, despacho generalizado + `trocarJogo`), `885fa6b` (T5, `SalaDeJogo` sobre registro), `a378972` (T6, `index.ts` valida/persiste `jogoId`). Confirmado depois do merge: `npm run typecheck` limpo, **495 testes unitários + 84 de integração**, todos verdes.
  - **Achado pelo worker, fora do escopo original das tasks mas necessário**: `Projecao.sala.jogoId` obrigatório (adicionado em T1) obrigou ripple em `server/games/quem-sou-eu/projecao.ts`/`projecao.test.ts`, e `EstadoSala.jogoId` obrigatório obrigou ripple mecânico de fixtures em `chat.test.ts`, `conexoes.integration.test.ts`, `estado.integration.test.ts`, `prazos.test.ts`, `roster.test.ts` — nenhum desses arquivos estava listado em `tasks.md`, mas eram consequência direta e inescapável dos campos novos serem obrigatórios (não opcionais) no protocolo. Não é dívida, é o mesmo padrão de ripple já visto em T2 de `pacotes-avancados`.
  - **Lição de processo**: a primeira tentativa de despachar o Lote A falhou porque `spec.md`/`design.md`/`tasks.md` da feature estavam **untracked** no checkout principal — um worktree isolado nasce só do que já está commitado, então o worker não enxergou nada da feature. Corrigido commitando os três arquivos (`74e5c97`) antes de redespachar. **Ao usar `isolation: worktree`, sempre commitar (ainda que em WIP) os arquivos que o worker precisa ler antes de despachar** — não basta eles existirem na working tree do checkout principal.
- **Próximo passo ao retomar**: Lote B (T7–T9, cliente) — `client/src/componentes/SeletorDeJogos.tsx` (novo), `client/src/telas/Inicio.tsx`, `client/src/telas/Lobby.tsx`. Sem Design Handoff visual (decisão registrada no próprio `design.md`: reaproveita `.pacote-card`/`Modal.tsx` existentes, não há tela nova o suficiente pra justificar). Depois do Lote B, a feature está com todas as 9 tasks feitas — dispara o Verifier automaticamente (não é opcional, é o próprio fluxo do skill).
