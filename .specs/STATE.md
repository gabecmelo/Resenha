...
- **Próximo passo ao retomar**: fase Tasks de `hub-selecao-jogos` (quebrar `design.md` em tasks atômicas — 14 requisitos, tocando `shared/protocolo.ts`, `shared/jogos-catalogo.ts` (novo), `server/core/sala-do.ts`, `server/core/despacho.ts`, `server/index.ts`, `server/games/registro.ts` (novo), `client/src/telas/Inicio.tsx`, `client/src/telas/Lobby.tsx`, `client/src/componentes/SeletorDeJogos.tsx` (novo) — provavelmente >8 tasks, oferecer sub-agents por batch antes de Execute). Sem Design Handoff visual (decisão registrada no próprio `design.md`: reaproveita `.pacote-card`/`Modal.tsx` existentes, não há tela nova o suficiente pra justificar).
