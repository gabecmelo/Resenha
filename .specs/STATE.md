...
- **`tasks.md`**: todas as 18 tasks marcadas `Done` (commit `7a06dc9`). Feature completa do lado de implementação.
- **Verifier independente — disparado, em andamento.** Sub-agent fresco (author ≠ verifier), rodando sem `isolation: worktree` (é read-only sobre a árvore real, mutação de teste só em scratch state via `git stash`). Vai escrever `.specs/features/pacotes-avancados/validation.md`. **Ao retomar, checar se esse arquivo já existe** — se sim, ler o veredito (PASS/FAIL) antes de qualquer outra coisa; se houver gaps, o próximo passo é rodar até 3 iterações de fix→re-verify antes de escalar pro dono.
- **In-progress** (file:line): none no checkout principal — o trabalho ativo é só o Verifier, em background, sem tocar arquivos além do relatório.
