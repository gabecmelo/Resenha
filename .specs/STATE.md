...
- **Dívida descoberta ao abrir esta rodada**: `pacotes-de-cartas` — `.specs/features/pacotes-de-cartas/` — está **implementada e em produção** (10 pacotes, seleção de modo, distribuição automática/escolha, KV com fallback estático, tudo confirmado lendo o código), mas foi feita fora do fluxo deste skill (outra ferramenta de IA, branch própria, PR #1, merge direto). `tasks.md` daquela feature tem todas as checkboxes vazias e este Handoff nunca a mencionou. Nenhum `validation.md` existe para ela. Não é bloqueante, mas qualquer rodada futura que mexer em pacotes deve tratar o código como fonte de verdade, não o `tasks.md` — e considerar rodar uma validação retroativa se o tempo permitir
