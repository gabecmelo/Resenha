...
- **Batch B (T10–T18, cliente) — disparado, em andamento.** Mesmo padrão de `isolation: worktree` + merge `--ff-only` ao final. Fecha o `typecheck` do lado cliente (`Lobby.tsx`, `Jogo.tsx`, `Encerrada.tsx`) e entrega grid responsivo, multi-seleção de pacote, seletor de dificuldade, "Ver pacote" (cálculo local via `shared/pacotes.ts`, sem round-trip), badge múltiplo, e os sons/micro-transição de clique (`FBK-*`). **Ao retomar, verificar se este batch já terminou** (procurar por commits depois de `cc4ebef` na branch, ou uma notificação de conclusão) — se sim, repetir o padrão de merge acima antes de prosseguir. Depois dele: o Verifier automático roda (author ≠ verifier, sensor de discriminação, `.specs/features/pacotes-avancados/validation.md`) — isso ainda não aconteceu.
- **In-progress** (file:line): none no checkout principal — o trabalho ativo está no worker do Batch B, em worktree separado.
